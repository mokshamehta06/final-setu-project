...
// Use mailtrapSender for more reliable testing
const { sendEmail } = require('../utils/mailtrapSender');
      ...
      <div class="container">
        <div class="card">
          ...
          <div class="content">
            ...
            <form action="/password-reset/forgot-password" method="POST" class="form">
        ...
        <div class="container">
          <div class="card">
            ...
            <div class="content">
              ...
              <form action="/password-reset/reset-password" method="POST" class="form" id="resetForm">
                <div class="form-group">
...
router.post('/forgot-password', async (req, res) => {
  try {
    ...
    if (!user) {
      return res.send(`
        <div style="text-align: center; margin-top: 50px;">
          ...
          <a href="/password-reset/${userType}/forgot-password">Go Back</a>
        </div>
      `);
    }
    ...
    // Create reset URL
    const resetUrl = `${req.protocol}://${req.get('host')}/password-reset/reset-password/${resetToken}`;
    ...
    try {
      ...
      // Always show the direct reset link on the page, regardless of email delivery
      return res.send(`
        <div style="text-align: center; margin-top: 50px;">
          <h1>Password Reset</h1>
          <p>We've attempted to send a password reset link to your email (${user.email}).</p>
          <p>If you don't receive the email, you can use the direct link below:</p>
          <a href="${resetUrl}" style="display: inline-block; background-color: #4CAF50; color: white; padding: 10px 20px; text-decoration: none; border-radius: 5px; margin-top: 20px; margin-bottom: 20px;">Reset Password</a>
          <p>This link will expire in 1 hour for security reasons.</p>
          <p>If you didn't request this password reset, you can safely ignore it.</p>
          <p style="margin-top: 20px;"><a href="/auth/${userType}/login">Back to Login</a></p>
        </div>
      `);
    } catch (error) {
      ...
      return res.send(`
        <div style="text-align: center; margin-top: 50px;">
          <h1>Password Reset</h1>
          <p>We encountered an issue sending the email, but you can still reset your password.</p>
          <p>Please use the direct link below:</p>
          <a href="${resetUrl}" style="display: inline-block; background-color: #4CAF50; color: white; padding: 10px 20px; text-decoration: none; border-radius: 5px; margin-top: 20px; margin-bottom: 20px;">Reset Password</a>
          <p>This link will expire in 1 hour for security reasons.</p>
          <p>If you didn't request this password reset, you can safely ignore it.</p>
          <p style="margin-top: 20px;"><a href="/auth/${userType}/login">Back to Login</a></p>
    ...
    // Try to generate a token anyway for a better user experience
    try {
      ...
      if (user) {
        ...
        return res.send(`
          <div style="text-align: center; margin-top: 50px;">
            <h1>Password Reset</h1>
            <p>We encountered an issue, but you can still reset your password.</p>
            <p>Please use the direct link below:</p>
            <a href="${resetUrl}" style="display: inline-block; background-color: #4CAF50; color: white; padding: 10px 20px; text-decoration: none; border-radius: 5px; margin-top: 20px; margin-bottom: 20px;">Reset Password</a>
            <p>This link will expire in 1 hour for security reasons.</p>
            <p>If you didn't request this password reset, you can safely ignore it.</p>
    ...
    // If recovery failed, show the standard error message
    return res.send(`
      <div style="text-align: center; margin-top: 50px;">
        <h1>Error</h1>
        <p>An error occurred. Please try again.</p>
        <a href="/password-reset/${req.body.userType || 'customer'}/forgot-password">Go Back</a>
      </div>
    `);
  }
});
    ...
    if (password !== confirmPassword) {
      return res.send(`
        <div style="text-align: center; margin-top: 50px;">
          ...
          <a href="/password-reset/reset-password/${token}">Go Back</a>
        </div>
      `);
    }
